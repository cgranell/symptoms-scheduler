...
data_path <- here::here("data", "data_ubicomp.csv")
write_csv(data_complete, data_path)
data_path <- here::here("data", "data_ubicomp.rds")
...
assessment_file <- here::here("data", "data_ubicomp.rds")
data <- readRDS(assessment_file)

data_perdevice <- readRDS(assessment_file)
...
Now, we compute statistics per device only, that is, data points from basic and advance schedulers are aggregated together to calculae descriptive statistics. 
...
data_perdevice %>%
  filter(abs(delay) < 2, device_id %in% c("BQ", "H9")) %>% 
