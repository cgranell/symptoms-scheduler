...
- `r scales::comma(n_observations, big.mark = ".")` observations in total. 

- `r round((n_irq / n_observations)*100, 1)`% (`r scales::comma(n_irq, big.mark = ".")`/`r scales::comma(n_observations, big.mark = ".")`) represents the middle 50% of the data, i.e. the interquartile range or [IQR](https://en.wikipedia.org/wiki/Interquartile_range)

- `r round((n_whisker / n_observations)*100, 1)`% (`r scales::comma(n_whisker, big.mark = ".")`/`r scales::comma(n_observations, big.mark = ".")`) of whisker values, i.e. the reasonable extremes of the data. That is, these are the minimum and maximum values that do not exceed 1.5 × IQR.

- `r round((n_outliers / n_observations)*100, 1)`% (`r scales::comma(n_outliers, big.mark = ".")`/`r scales::comma(n_observations, big.mark = ".")`) of outliers, i.e.beyond the whiskers.
...
- `Xiaomi Mi A1` has stopped triggering alarms on **2020-04-23 00:52:00** when running the basic scheduler. 


```{r outliers_table_plot}

outliers <- 
  filter(data, outlier==1) %>%
  group_by(exp_id, scheduler, device_name) %>%
  summarise(`# outliers` = n()) %>%
  # select (exp_id, scheduler, device_name, delay) %>%
  arrange(exp_id, device_name)

kable(outliers,
      format = "html",
      booktabs = TRUE,
      caption = "Table. Outliers total per sheduler/device.") %>%
  kable_styling(full_width =FALSE,  position = "left")


ggplot(outliers) +
  geom_bar(aes(x=device_name, y=`# outliers`, fill=scheduler), stat = 'identity', position = position_dodge(preserve = 'single')) +
  theme_bw()
...
```{r boxplot_paired, fig.height=6, fig.width=8}
