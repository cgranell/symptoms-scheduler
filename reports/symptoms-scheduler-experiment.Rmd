...
- Which one of the analysed schedulers is more accurate? (lower delay)
__- Which one of the analysed schedulers is more reliable? (fewer outliers and a more predictable execution)__
- Which one of the analysed schedulers loses fewer alarm executions? (higher percentage of alarms executed / expected alarms for the experiment time frame)
- Does the battery level influence timely alarm execution?
...
_Which one of the analysed schedulers is more reliable?_ To answer this question we analyse the sitribtu of outleide per devicewhich scheduler shows fewer outliers, becaure this signals a more predictable execution,
...
_How do outliers distribute over time?_


```{r outliers_BQ}

n_outliers_BQ <- nrow(filter(data, outlier==1, device_id =="BQ"))
n_outliers_pos_BQ <- nrow(filter(data, outlier_pos==1, device_id =="BQ"))
n_outliers_neg_BQ <- nrow(filter(data, outlier_neg==1, device_id =="BQ"))

outliers_BQ <- 
  filter(data, outlier==1, device_id =="BQ") %>%
...
outliers_pos_BQ <- 
  filter(data, outlier_pos==1, device_id =="BQ") %>%
  group_by(scheduler, plan_month, plan_day, plan_hour) %>%
  summarise(n = n()) %>%
  mutate(date = lubridate::ymd(paste0("2020-",
                                        str_pad(plan_month, width=2, side="left", pad="0"),"-", 
...
outliers_neg_BQ <- 
  filter(data, outlier_neg==1, device_id =="BQ") %>%
  group_by(scheduler, plan_month, plan_day, plan_hour) %>%
  summarise(n = n()) %>%
  mutate(date = lubridate::ymd(paste0("2020-",
                                        str_pad(plan_month, width=2, side="left", pad="0"),"-", 
...
outliers_BQ_bubblechart <- 
    ...
    theme_minimal() +
    labs(title= "Count of outliers (ALL) by hour")
...
outliers_pos_BQ_bubblechart <- 
    ...
    scale_x_date(breaks = scales::date_breaks("1 day"),
                 labels = scales::date_format("%d-%m", tz="CET")) +
    theme_minimal() +
    labs(title= "Count of outliers (POS) by hour")
...
outliers_neg_BQ_bubblechart <- 
    ...
    scale_x_date(breaks = scales::date_breaks("1 day"),
                 labels = scales::date_format("%d-%m", tz="CET")) +
    theme_minimal() +
    labs(title= "Count of outliers (NEG) by hour")
