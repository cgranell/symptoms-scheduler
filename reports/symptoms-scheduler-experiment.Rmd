...
`r scales::comma(n_observations, big.mark = ".")` observations registered in both experiments. Table below summarises ranges and total numbers per device. 
...
`Basic - BQ Aquaris V` and `Basic - Nvidia Shield Tablet` do not show outliers! 

Some devices present quite distint numbers of outliers dependinng on the scheduler.  
...
```{r plot_battery_delay, fig.height=10}

p_battery_delay <-
  battery_delay_data %>%
  ggplot(aes(x = battery, y =delay)) + 
  geom_point(aes(color=device_name), alpha = 0.5, size = 0.1) +
  facet_grid(device_name ~ scheduler, margins = TRUE) +
  theme_bw() +
  guides(colour=FALSE)
