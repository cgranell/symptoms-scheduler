...
In mental health interventions, to have a predictable execution of planned actions is mostly preferred which means that notifications and/or alerts are delivered to patients at the right moment. It is inconceivable, for example, that a patient gets a notification (e.g. action, psyco content, etc.) 1 minute later than the event (e..g passing nearby a gambling  hall) that triggered it; the patient could be dozens of meters away from the place of interest or, even worse, walked into that "no-desirable" place. Therefore, closing the "temporal" gap between the triggered event and the action is critical (time matters), especially because time and space/place must be in synchrony in mental health EMIs. If these two dimensions are not in synchrony, the intervetion becomes far less effective.

Given that average response time (delay) is similar in all devices (< 1 second), outliers are a good candidate to study reliability. In other words, if a scheduler leads to fewer outliers (in terms of delay) would mean that planned actions are more stable and predictable over time/space and, consequently, that scheduler does a good job. On the other hand, a high number of and/or frequent outliers would signal lack of synchrony between space/place and time dimensions in an intervention, resulting in a less suitable scheduler for EMI/A.
...
- `Honor 9` shows a remarkly difference between the two schedulers. This is importnat because this device was used regularly while runing the experiments, demonstrted by the distinct distribution of outliers atnigthtime againts daytime. 

- Majority of outliers are positive (beyond top whisker). Negative values should be interpreted as wrong values due to planned timestamps mistakenly recorded during data collection.
...
### Q2 (Ongoing in another notebook. Will be integrated shortly here)
