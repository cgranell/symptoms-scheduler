...
### Which one of the analysed schedulers is more accurate? (Distribution of `delay` - without outliers)
...
selection <- 
  data %>%
  filter(device_name == sel_device_name) %>%
  arrange(plan_date) %>%
...
selection %>%
  ...
  mutate(plan_date_lagged = lag(plan_date),
         ...
         plan_date_cum = cumsum(plan_date_diff),
         exec_date_lagged = dplyr::lag(exec_date),
         exec_date_diff = as.duration(interval(exec_date_lagged, exec_date)) / dseconds(1),
         exec_date_diff = round(exec_date_diff, 3)) -> selection

# TODO: some weird values. some values of plan_date_diff > 100, 1 value > 1000!)
selection %>%
  ggplot(aes(x = device_name, y = plan_date_diff, fill=factor(scheduler))) + 
