...
- `r scales::comma(n_observations, big.mark = ".")` observations registered in both experiments. Table above summarises ranges and total numbers per device. 
- `r devices["NV"]` has stopped triggering alarms on  **2020-04-17 22:27:06** when running the basic scheduler. Since there's no _watchdog_ in the basic version, this device is not going to trigger alarms anymore. Died!
...
summaries <- 
  ...
  summarise(`# observations` = n(),
            ...
            `mean - 2sd` = first(lo),
            `mean + 2sd` = first(hi)) %>%
  arrange(exp_id, scheduler, device_name)
...
kable(summaries,
  ...
  row_spec(9, color = "white", background = "#D7261E")

```

```{r}
stats <- data %>% filter(exp_id=="#1", device_name=="BQ Aquaris V") %>% select(delay)
 summary(stats)
#TODO: ERROR EN EL CALCUALO DE LAS MEDIAS
...
- `r round((n_outliers / n_observations)*100, 1)`% (`r scales::comma(n_outliers, big.mark = ".")`/`r scales::comma(n_observations, big.mark = ".")`) of outliers. Outlier breakdown below. 
...
See the [shiny app](https://cgranell.shinyapps.io/symptoms-scheduler/)!


### Distribution of `delay` over `battery` (without outliers)

```{r battery_delay_data}
...
```
```{r boxplot_all_faceted, fig.height=6, fig.width=5}
