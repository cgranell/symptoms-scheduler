...
```{r histogram fig.height=8, fig.width=10}
...
performance %>%
  ...
  # facet_wrap(scheduler~device_id, ncol = 4, nrow = 2) +
  labs(title="", x ="Devices", y="Delay [seconds]") +
  geom_text(data = missingness_basic, aes(x = device_id, y = 10, label = percent), 
            nudge_y= 0.5,nudge_x = 0.2) +
  geom_text(data = missingness_advanced, aes(x = device_id, y = 10, label = percent), 
            nudge_y= 0.5,nudge_x = -0.2) +
  theme_minimal() -> p
