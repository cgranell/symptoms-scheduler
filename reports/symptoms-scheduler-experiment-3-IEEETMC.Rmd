...
But, A1-BA and NV-BA did **not** end the experiment. The expected number of executions are

- A1-BA: `r n_plan_exec_a1ba` instead of `r filter(missingness, device_id =='A1' & scheduler=='Basic') %>% select(n_plan_exec)`.

- NV-BA: `r n_plan_exec_a1ba` instead of `r filter(missingness, device_id =='NV' & scheduler=='Basic') %>% select(n_plan_exec)`.
...
devices_os <- 
  missingness %>% 
  dplyr::filter(scheduler == "Advanced") %>% 
